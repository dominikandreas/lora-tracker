...
test('uses received time for legacy and malformed timestamp fields', () => {
  const legacy = normalizePoint({ ...base, point_schema_version: 1, received_at_ms: 1234 }, 99);
  const malformed = normalizePoint({ ...base, timestamp_valid: true, fix_time_unix_ms: 1.5 }, 99);
  assert.equal(legacy.effective_time_unix_ms, 1234);
  assert.equal(legacy.time_source, 'received');
