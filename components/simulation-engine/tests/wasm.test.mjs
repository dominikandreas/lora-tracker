...
import { FirmwareCore, ReferenceCore } from "../src/firmware-core.js";

async function loadWasm() {
  const bytes = await readFile(
    new URL("../dist/firmware-core.wasm", import.meta.url),
  );
...
test("WASM executes the same golden policy vectors as the reference contracts", async () => {
  ...
  for (const radio of radios) {
    ...
    for (const bytes of [1, 56, 128, 255])
      assert.equal(
        wasm.airtimeMs(bytes, radio),
        reference.airtimeMs(bytes, radio),
      );
    assert.ok(
      Math.abs(wasm.sensitivityDbm(radio) - reference.sensitivityDbm(radio)) <
        1e-9,
    );
    assert.equal(
      wasm.ackRelayGuardMs(255, radio, 0, 2),
      reference.ackRelayGuardMs(255, radio, 0, 2),
    );
  }
  assert.equal(
    wasm.validateRadio({ ...radios[0], spreadingFactor: 263 }),
    false,
  );
  ...
  for (const vector of [
    ...
    assert.equal(
      wasm.sleepSeconds(config, ...vector),
      reference.sleepSeconds(config, ...vector),
    );
  }
  for (const failures of [0, 1, 2, 3, 9])
    assert.equal(
      wasm.retrySeconds(config, failures),
      reference.retrySeconds(config, failures),
    );
